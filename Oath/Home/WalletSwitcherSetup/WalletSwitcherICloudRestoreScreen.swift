...
struct WalletSwitcherICloudRestoreScreen: View {
  let database: WalletDatabase

  @Environment(\.scenePhase) private var scenePhase
  @Environment(\.accessibilityReduceMotion) private var reduceMotion
  @State private var discovery: WalletSwitcherICloudDiscoveryModel
  ...
  init(
    ...
    discovery: WalletSwitcherICloudDiscoveryModel = .init()
  ) {
      ...
      .fullScreenCover(
        ...
        if let context = deletionAuthenticationContext {
          WalletAuthenticationFullScreenContainer(
            ...
            WalletSwitcherBackupDeletionAuthenticationScreen(
              database: database,
              context: context
            ) {
  ...
  private func backupListRow(
    ...
    // Editing uses a label so selection never inherits disabled-link styling.
    VStack(alignment: .leading, spacing: 0) {
      if editMode.isEditing {
        backupRow(backup: backup)
          .accessibilityElement(children: .combine)
      } else {
        NavigationLink(value: WalletSwitcherNavigationRoute.setup(.restoreBackup(backup))) {
          backupRow(backup: backup)
        }
      }
    }
