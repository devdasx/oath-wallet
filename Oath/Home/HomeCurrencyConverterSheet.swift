...
enum HomeCurrencyConverterSheetDetentPolicy {
    static let allowedDetents = Set<PresentationDetent>([.large])
}

struct HomeCurrencyConverterSheet: View {
            ...
            List {
                Group {
                    if isLoading && units.isEmpty {
                        ...
                        Section {
                            HomeCurrencyConverterFailureView {
    ...
    private var converterSections: some View {
        Section {
            ...
            if canAddUnit {
                NavigationLink {
                    Group {
                        HomeCurrencyConverterUnitSelectionScreen(
                            units: units,
                            selectedID: "",
    ...
    private func amountField(
        ...
        HStack(spacing: 12) {
            HomeCurrencyConverterAmountLogo(unit: unit)
...
private struct HomeCurrencyConverterAmountLogo: View {
...
private struct HomeCurrencyConverterFailureView: View {
    let retry: () -> Void
