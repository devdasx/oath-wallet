...
enum SendRecipientScanError: Error, Equatable, Sendable {
    case invalidForSelectedNetwork

    func localizedMessage(networkName: String) -> String {
        switch self {
        case .invalidForSelectedNetwork:
            EnglishNumbers.localized(
                "send.recipient.scan.error.invalid_network",
                networkName
            )
        }
    }
...
enum SendRecipientScanPreparation {
    static func recipient(
        ...
        let recipient = request.recipient.trimmingCharacters(
            in: .whitespacesAndNewlines
        )

        guard
            request.source != .name,
            request.candidateNetworkIDs.contains(selectedNetworkID),
            SendAddressValidator.isValid(
                recipient,
                for: selectedNetworkID
            )
        else {
            throw SendRecipientScanError.invalidForSelectedNetwork
        }
...
struct SendRecipientScannerScreen: View {
    ...
    var body: some View {
        ...
        .navigationTitle("send.recipient.scan.title")
            ...
            if let recognitionError {
                ...
                Text(
                    verbatim: EnglishNumbers.localized(
                        "send.recipient.scan.instruction",
                        asset.networkName
                    )
                )
    ...
    private func recognize(_ payload: String) -> Bool {
        do {
            let recipient = try SendRecipientScanPreparation
                .recipient(
                    from: payload,
                    selectedNetworkID: asset.networkID
                )
            recognitionError = nil
            onRecognized(recipient)
            UniHaptic.play(.successQuiet)
            return true
        } catch let error as SendRecipientScanError {
