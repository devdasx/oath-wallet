...
private enum SettingsBackupPrivateKeyRoute: Hashable, Identifiable {
    case display(String)
    case bitcoinTypes
...
struct SettingsBackupPrivateKeyFlow: View {
    let wallet: ManagedWallet

    @Environment(\.scenePhase) private var scenePhase
    @Environment(\.dismiss) private var dismiss
    @State private var privateKeyRoute: SettingsBackupPrivateKeyRoute?
    ...
    private var rootContent: some View {
        if wallet.kind.hasRecoveryPhrase {
            SettingsBackupPrivateKeySelectionScreen(
                items: items,
                onSelect: {
    ...
    private func destination(
        for route: SettingsBackupPrivateKeyRoute
    ) -> some View {
        switch route {
        case let .display(itemID):
        ...
        case .bitcoinTypes:
            if let catalog = bitcoinCatalog {
                SettingsBackupBitcoinKeyTypeSelectionScreen(
                    wallet: wallet,
                    catalog: catalog,
