...
struct WalletSwitcherCredentialScannerScreen: View {
    ...
    @State private var review: ImportCredentialScanReview?
    ...
    var body: some View {
        NavigationStack {
            Group {
                ZStack {
                    if let review {
                        SmartScannerReviewPanel(
                            ...
                            onConfirm: {
                                onRecognized(review.normalizedValue)
                            },
                ...
                .navigationTitle("import.scanner.navigation.title")
    ...
    private var scannerMessage: some View {
        ZStack {
            if let recognitionError {
                ...
                Text("import.scanner.instruction")
                    .foregroundStyle(.secondary)
            }
        }
        .font(.subheadline)
    ...
    private func recognize(_ payload: String) -> Bool {
        do {
            review = try ScannerPayloadPolicy.importCredential(
                from: payload,
                mode: mode
            )
